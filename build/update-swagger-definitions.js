...
agolSchemas.getSchema('swaggerDefinitions').then(function (defs) {
  fs.readFile(path.resolve(__dirname, '..', 'api/swagger/swagger.json'), function (error, data) {
    if (error) throw error;
    var swaggerDoc = JSON.parse(data);
    swaggerDoc.definitions = defs;
    fs.writeFile(path.resolve(__dirname, '..', 'api/swagger/swagger.json'), JSON.stringify(swaggerDoc, null, 2), function () {
